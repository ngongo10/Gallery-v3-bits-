...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    const fieldProgram = new Program(gl, {
      vertex: FIELD_VERT, fragment: FIELD_FRAG,
    ...
    const compProgram = new Program(gl, {
      vertex: SCREEN_VERT, fragment: COMP_FRAG,
      uniforms: {
        tField: { value: null },
    ...
    const spawn = (i, ox, oy) => {
      ...
      for (let h = 0; h < HISTORY; h++) { histX[h * MAX + i] = px[i]; histY[h * MAX + i] = py[i]; }
    };
    for (let i = 0; i < MAX; i++) {
    ...
    // Use window so tracking works even though the overlay is pointer-events:none
    const onMove = e => { const r = container.getBoundingClientRect(); cursor.x = e.clientX - r.left; cursor.y = e.clientY - r.top; cursor.has = true; };
    ...
    const onDown = e => {
      ...
      for (let i = 0; i < MAX; i++) {
        let dx = px[i] - cx, dy = py[i] - cy, d = Math.hypot(dx, dy);
    ...
    const frame = now => {
      ...
      if (!p.enabled || reduceMotion) { renderer.render({ scene: compMesh }); return; }
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
          if (j === i) continue;
          const sx = px[i] - px[j], sy = py[i] - py[j], d2 = sx * sx + sy * sy;
          if (d2 > 1e-4 && d2 < sepDist * sepDist) {
            const d = Math.sqrt(d2), f = (1 - d / sepDist) * maxSpeed * 3.2 * p.separation;
            ax += (sx / d) * f; ay += (sy / d) * f;
          }
        }

        const al = Math.hypot(ax, ay), cap = maxForce * (1 + burst * 4);
      ...
      for (let i = 0; i < n; i++) {
        const headR = p.size * scale[i] * 2.1, headW = 1.06 + 0.3 * scale[i];
        pushQuad(px[i], py[i], headR, headW);
        if (trailAge < 0.01 || maxStamps < 2 || histLen < 2) continue;
        const step = Math.max(2, p.size * scale[i] * 0.5), span = step * maxStamps;
        ...
        for (let j = 0; j < histLen && stamps < maxStamps; j++) {
          ...
          const segX = hx - prevX, segY = hy - prevY, segLen = Math.hypot(segX, segY);
          if (segLen < 1e-4) continue;
          while (nextAt <= walked + segLen && stamps < maxStamps) {
            const f = (nextAt - walked) / segLen, u = nextAt / span;
            const taper = Math.pow(Math.max(0, 1 - u), 0.55), rLocal = headR * taper;
